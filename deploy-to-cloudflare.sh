...
# Ensure we're using Node.js 20 or higher
NODE_VERSION=$(node -v | cut -d 'v' -f2 | cut -d'.' -f1)
if [ "$NODE_VERSION" -lt 20 ]; then
  echo "Error: Node.js version 20 or higher is required. Current version: $(node -v)"
  echo "Please install a compatible version of Node.js before continuing."
  
  if command -v nvm &> /dev/null; then
    echo "Detected NVM. Would you like to use it to switch to Node.js 20? (y/n)"
    read -r USE_NVM
    if [ "$USE_NVM" = "y" ]; then
      nvm install 20
      nvm use 20
